...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (createNewSlipBtn) {
        createNewSlipBtn.addEventListener('click', () => {
            // (★修正★) all-slips.js では tableId が取得できないため、このボタンは機能しない
            // (ただし、コードとしては残し、実行時エラーも発生しない)
            const tableId = createNewSlipBtn.dataset.tableId;
            if (tableId) {
                // createNewSlip(tableId); // createNewSlip はこのファイルにないので呼び出せない
                closeModal(slipSelectionModal);
            }
        });
    }
